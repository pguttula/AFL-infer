...
main(void)
{
    ...
    */
    ...
    if(scanf("%lu",&bin_len) == EOF) {
      return -1;
    }
    if(scanf("%lu",&blocksize) == EOF) {
      return -1;
    }
    if(scanf("%lu",&bin_padded_len) == EOF) {
      return -1;
    }
    if(scanf("%lu",&bin_padded_maxlen) == EOF) {
      return -1;
    }
    bin_padded = (unsigned char *) sodium_malloc(bin_padded_len);
    sodium_pad(&bin_padded_len, bin_padded, bin_len, blocksize, bin_padded_maxlen);
    /*
    for (i = 0; i < 2000U; i++) {
